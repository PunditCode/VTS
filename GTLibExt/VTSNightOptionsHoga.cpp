...
VTSNightOptionsHoga::VTSNightOptionsHoga(void) 
{
}

BOOL VTSNightOptionsHoga::RunRecv()
{
	int nRet = SER()->RegisterHoga(m_pCode, mt_EUREX, this);
...
void VTSNightOptionsHoga::StopRecv()
{
	SER()->UnregisterHoga(m_pCode, mt_EUREX, this);
}
...
BOOL VTSNightOptionsHoga::OnInitReceive(CHAR* pPacket, UINT nDataLength)
{

	if(nDataLength != sizeof(t2831OutBlock))
		return FALSE;

	LPt2831OutBlock p = (LPt2831OutBlock)pPacket;
...
BOOL VTSNightOptionsHoga::OnReceive(CHAR* pPacket, UINT nDataLength)
{
	LPEH0_OutBlock p = (LPEH0_OutBlock)pPacket;
