...
VTSNightFuturesHoga::VTSNightFuturesHoga(void) 
{
}

BOOL VTSNightFuturesHoga::RunRecv()
{
	int nRet = SER()->RegisterHoga(m_pCode, mt_CME, this);
...
void VTSNightFuturesHoga::StopRecv()
{
	SER()->UnregisterHoga(m_pCode, mt_CME, this);
}
...
BOOL VTSNightFuturesHoga::OnInitReceive(CHAR* pPacket, UINT nDataLength)
{

	if(nDataLength != sizeof(t2802OutBlock))
		return FALSE;

	LPt2802OutBlock p = (LPt2802OutBlock)pPacket;
...
BOOL VTSNightFuturesHoga::OnReceive(CHAR* pPacket, UINT nDataLength)
{
	LPNH0_OutBlock p = (LPNH0_OutBlock)pPacket;
