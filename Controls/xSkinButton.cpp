...
//	m_Brush.CreateSolidBrush(COLOR_DEFAULT_BACK);
...
HBRUSH CxSkinButton::CtlColor(CDC* pDC, UINT nCtlColor) 
{
...
{	
	CRect rt;
	GetClientRect(rt);
	pDC->FillSolidRect(rt, COLOR_DEFAULT_BACK);
	return 1; 
...
void CxSkinButton::DrawItem(LPDRAWITEMSTRUCT lpDrawItemStruct) 
{
	...
	CRect r=lpDrawItemStruct->rcItem;					// context rectangle
	
	CRect r2(r);
	ClientToScreen(r2);
	
	int cx = r.Width();						// get width
	int cy = r.Height();						// get height
		...
		{											// DEFAULT BUTTON
			if(m_bNormal.IsNull())
				// no skin selected for normal state -> standard button
					pDC->FillSolidRect(&r,GetSysColor(COLOR_BTNFACE));
			else if ((m_tracking)&&(m_bOver.IsNull() == false))
			{
				m_bOver.Draw(pDC->GetSafeHdc(), r);
			} 
			else 
			{
				if ((lpDrawItemStruct->itemState & ODS_FOCUS)&&(m_bFocus.IsNull() == false))
				{
					m_bFocus.Draw(pDC->GetSafeHdc(), r);
				} 
				else 
				{
					m_bNormal.Draw(pDC->GetSafeHdc(), r);
				}
			}
				// if needed, draw the standard 3D rectangular border
			if (m_Border) 
...
void CxSkinButton::DrawBitmap(CDC* dc, HBITMAP hbmp, RECT r, int DrawMode)
{
	...
	if(FALSE == hbmp) return;	//safe check
...
/////////////////////////////////////////////////////////////////////////////
void CxSkinButton::SetSkin(LPCTSTR normal,LPCTSTR down,LPCTSTR over,LPCTSTR disabled, LPCTSTR focus, COLORREF crTransparent,
						   short drawmode, short border, short margin)
{
	bool bHadAlphaChannel = true;
	if(normal && SUCCEEDED(m_bNormal.Load(normal)))
	{
		m_bNormal.SetTransparentColor(crTransparent);
		m_bNormal.SetHasAlphaChannel(bHadAlphaChannel);
	}

	if(down && SUCCEEDED(m_bDown.Load(down)))
	{
		m_bDown.SetTransparentColor(crTransparent);
		m_bDown.SetHasAlphaChannel(bHadAlphaChannel);
	}

	if(over && SUCCEEDED(m_bOver.Load(over)))
	{
		m_bOver.SetTransparentColor(crTransparent);
		m_bOver.SetHasAlphaChannel(bHadAlphaChannel);
	}

	if(disabled)
	{
		if(SUCCEEDED(m_bDisabled.Load(disabled)))
		{
			m_bDisabled.SetTransparentColor(crTransparent);
			m_bDisabled.SetHasAlphaChannel(bHadAlphaChannel);
		}
	}
	else
	{
	...
	if(focus && SUCCEEDED(m_bFocus.Load(focus)))
	{
		m_bFocus.SetTransparentColor(crTransparent);
		m_bFocus.SetHasAlphaChannel(bHadAlphaChannel);
	}
	...
	if (m_DrawMode==0)
	{
		SetWindowPos(NULL, 0, 0, m_bNormal.GetWidth(), m_bNormal.GetHeight(), SWP_NOZORDER|SWP_NOMOVE);
	}
}
/////////////////////////////////////////////////////////////////////////////
