...
CEnTabCtrl::CEnTabCtrl() : m_dwCustomLook(0), m_crSelectTab(-1)
{
	m_crTextColorSel = ::GetSysColor(COLOR_WINDOWTEXT);
	m_crTextColor = ::GetSysColor(COLOR_3DFACE);
	m_crNoneSelectTab = Lighter(::GetSysColor(COLOR_3DFACE), 1.4f);
}
...
void CEnTabCtrl::DrawItem(LPDRAWITEMSTRUCT lpdis)
{
	...
	BOOL bColor = (m_dwCustomLook & ETC_COLOR);
	...
	{
		COLORREF crTo = bSelected ? Darker(!bColor || m_crBack == -1 ? ::GetSysColor(COLOR_3DFACE) : m_crBack, 0.2f) : ::GetSysColor(COLOR_3DFACE);
...
{
	BOOL bColor = (m_dwCustomLook & ETC_COLOR);
	BOOL bHiliteSel = (m_dwCustomLook & ETC_SELECTION);
	BOOL bBackTabs = (m_dwCustomLook & ETC_BACKTABS);
	BOOL bFlat = (m_dwCustomLook & ETC_FLAT);

	if (bSelected && bHiliteSel)
	{
		if (bColor)
			return m_crSelectTab;//Lighter((m_crSelectTab == -1) ? ::GetSysColor(COLOR_3DFACE) : m_crSelectTab, 1.4f);
		else
			return m_crNoneSelectTab;
	}
	else if (!bSelected)
	{
		return m_crNoneSelectTab;//Darker(::GetSysColor(COLOR_3DFACE), 0.9f);
	}
...
#else
		if (bColor || bFlat)
			return Darker((m_crBack == -1) ? m_crTextColor : m_crBack, 0.5f);
		else
			return Darker(m_crTextColor, 0.5f);
#endif
	}
