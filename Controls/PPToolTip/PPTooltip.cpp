...
CPPToolTip::CPPToolTip()
{
	...
	m_dwShowEffect = SHOWEFFECT_NONE; //SHOWEFFECT_FADEINOUT;
	m_dwHideEffect = SHOWEFFECT_NONE; //SHOWEFFECT_FADEINOUT;
...
//	EnableTextWrap(FALSE);
	...
	if(FALSE == (::GetClassInfo(hInst, PPTOOLTIP_CLASSNAME, &wndcls)))
	{
...
//	GetWindowRgn(hrgn);
	...
	//RUS: 嵌炡宗諺 典嵬妖贍茴渾 訟嫉涉 脣 荀諺孼裝?惟粧焉增 檉?震
	...
	//RUS: 昆幽雪顥齧?膣意寥 壯 幽猥粧仲策 妬增??贍橫?壯依蓮杖諺
	...
	//RUS: 蛟脣灑輦齧?惟粧焉增 埰疾迹增循 嚴泣 憙?張翟軸怏溢
	if (bAutoReleaseDC)
...
BOOL CPPToolTip::RelayEvent(MSG* pMsg)
{
	...
	//RUS: 행穽奄 翟調灑叱?櫛翟緘杖?嚴泣 翟衒攸 張 櫛醴陝
	if (NULL == GetSafeHwnd())  
		return FALSE;
				...
				else 
				{
					...
					else
					{
						...
						else
						{
							...
							//RUS: 凌意寥 張 哀飮孼 穽診潗? 禎魏 壯軸怏秩 壯?適狀?
							SetAutoPopTimer();
						} //if
					} //if
				} //if
...
void CPPToolTip::OnTimer(UINT nIDEvent) 
{
	...
	case TIMER_SHOW:
		...
		//ENG: Kill SHOW timer 
		//RUS: 帶嶢?診雩儼 跡巍陝? 禎魏裔 膣意寥?
		KillTimer(TIMER_SHOW);
		//ENG: Get current mouse coordinates
		//RUS: 淃懿婆潗 鎭有緘?禎依蓮杖?膣意寥?
	...
	case TIMER_SHOWING:
		...
		//RUS: 퇸泣 胞燼 戱輒攸?町設狀莘 禎魏裔 尿特卒陝? 桎 埰診狀淳潗 麟杖茵蟻腸?穽蹟調杷前潗
		...
		if (m_dwCurTransparency <= m_tiDisplayed.nTransparency)
		{
			...
			//RUS: 帶外齧?診雩儼 町設狀莘 禎魏裔 ?埰診壯瞬外齧?櫛增?杖?膣意寥?魏?SHOWN
	...
	case TIMER_HIDING:
		...
		//RUS: 퇸泣 胞燼 戱輒攸?町設狀莘 櫛愉泓? 尿特卒陝? 桎 埰診狀淳潗 茵揄夭贍茴策 穽蹟調杷前潗
		...
		if (PERCENT_MAX_TRANSPARENCY == m_dwCurTransparency)
		{
			...
			//RUS: 帶外齧?診雩儼 町設狀莘 櫛愉泓? ?穽特?適狀 膣意寥?
			...
			if (m_bNextToolExist)
			{
				...
				//RUS: 퇸泣 憎緘增詢奄 禎押剪荻音牆渾 膣意寥, 桎 壯妬潗 憶?剪翟調蓮杖?
				m_bNextToolExist = FALSE;
//				m_nTooltipType = m_nNextTooltipType;
//				m_nNextTooltipType = PPTOOLTIP_NORMAL;
...
void CPPToolTip::HideTooltip()
{
	...
	hdr.code     = UDM_TOOLTIP_HIDING; 

	::SendMessage(m_hNotifyWnd, WM_NOTIFY, hdr.idFrom, (LPARAM)&hdr);  
...
DWORD CPPToolTip::GetTooltipDirection(DWORD dwDirection, const CPoint & ptPoint, CPoint & ptAnchor, CRect & rcBody, CRect & rcFull, CRect & rcTipArea)
{
	...
	//RUS: 淃懿妬諺 禎鷹渾 穽溢昌鏑茴凹 僖調壯 Windows
	CRect rWindow;
	...
	//RUS: 흡擾歪泣裔打 禎鷹謫?調傲儼?謫調杖婆循憶?膣意寥 穽溢昌鏑茴凹?
	rcFull = rcBody;
	switch(dwDirection) 
	{
	case PPTOOLTIP_LEFTEDGE_TOP:
	...
	//RUS: 厥荻儼魏 莘鳥吾粧贍茴紅 調傲儼荻 壯 禎檉菴杖??僖調?
	CPoint pt(ptPoint);
	switch(dwDirection) 
	{
	case PPTOOLTIP_LEFTEDGE_TOP:
	...
	//RUS: 厥荻儼魏 橓倧凹贍茴紅 調傲儼荻 壯 禎檉菴杖??僖調?
	switch(dwDirection) 
	{
	case PPTOOLTIP_LEFTEDGE_TOP:
	case PPTOOLTIP_RIGHTEDGE_TOP:
...
BOOL CPPToolTip::FindTool(HWND hWnd, const LPPOINT lpPoint, PPTOOLTIP_INFO & ti)
{
	...
	//RUS: 淃懿妬諺 茵葺外 ?檉調靭疾閃?莘?婆?吾?瘡暹陝謫?適壯
	...
	if ((hotarea.size() == 1) && hotarea[0].rectBounds.IsRectEmpty())
	{
		...
		//RUS: 퇸泣 謫調杖婆循僥 穽溢昌鏑茴凹 莘?特?吾裝 張 全釣鴨音?
		ti = hotarea[0];
		return TRUE;
	} //if
	...
	if (hWnd != m_hParentWnd)
	{
		...
		//RUS: 퇸泣 HWND 張 剪狀茁秩 ?尊怏鎭蟻仲佃?適腸, 桎 穽蘖鎖暹暢?惟典怏壯執
	...
	if (pScroll->IsKindOf(RUNTIME_CLASS(CScrollView))) 
	{
		...
		//RUS: 퇸泣 HWND 穽妖雪音鍊?CScrollView 堯?穽迪譽賊狀壬 剪 張莘 幽葉憎, 桎 惟拙焉震宗諺 惟典怏壯執
...
HWND CPPToolTip::FindToolBarItem(POINT point, PPTOOLTIP_INFO & ti)
{
	...
	//RUS: 嵌粧尊蟻 裔 禎愛魏伍閃??檉張??妖增宗靭粧荻 剪幽孼
...
void CPPToolTip::AddTool(CWnd * pWnd, PPTOOLTIP_INFO & ti)
{
	...
	if (item == m_ToolMap.end())
	{
		...
		//RUS: 凌意寥 秧 瘡暹陝狀莘 HWND 張 翟壯宗蓮? 禎憙佃?櫛醴齧?憶?
		arHotArea hotarea;
	...
	//RUS: 凌意寥 櫛鴨阻嶢 灑音?賊狀?穽溢昌鏑茴迹 翟乙增? 穽荻儼夭 蓴嶪 撓
	arHotArea::iterator iter;
	for (iter = hotarea.begin(); iter != hotarea.end(); ++iter)
	{
		if (ti.rectBounds == (*iter).rectBounds)
		{
			...
			//RUS: 黛暹陝裝?穽溢昌鏑茴凹 適壯 槍?憎緘增詢奄, 禎憙佃?穽前桎 翟狀瞬諺 憶?檉調靭疾?
			*iter = ti;
			return;
		} //if
	} //for
		...
		for (iter = hotarea.begin(); iter != hotarea.end(); ++iter)
		{
			if ((*iter).rectBounds == *lpRectBounds)
			{
				...
				if (hotarea.size() > 1)
				{
					...
					//RUS: 퇸泣 瘡暹陝狀?適狀 櫛鴨阻嶢 灑音?賊狀?翟乙增? 桎 昶贍諺 桎蟻惟 瘡暹陝腸?翟乙增?
					hotarea.erase(iter);
				}
				else
				{
					//ENG: If the specified window has one rectangle area only then removes the tool for specified window
					//RUS: 퇸泣 瘡暹陝狀?適狀 夭檍?桎蟻惟 賊腸 穽溢昌鏑茴策 翟乙增? 桎 昶贍嶢?橓贈 妖增宗靭粧
					m_ToolMap.erase(item);
				} //if
				return;
			} //if
