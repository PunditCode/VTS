...
void CPPHtmlDrawer::DrawHtml (LPSIZE lpSize, LPCRECT lpRect)
{
	...
	//RUS: 嬌調杖婆循彦 穽溢昌鏑茴凹 秧 鎭有緘?翟乙增?述脣菴
	RECT rcArea;
...
//		m_arrTableSizes.clear();
	...
	while (nIndex < m_csHtmlText.GetLength())
	{
		...
		//RUS: 퇸泣 鎭揄?渟釣?診頌擾彦 憎緘增詢奄
		if (!strText.IsEmpty())
		{
			//ENG: Add a tag BODY around of a output text
			//RUS: 쿡蓀瞬諺 緝?BODY 脣愉昌 述脣怏溢莘 鎭揄診
...
//		sc.nWidth = 0;
	...
	//RUS: 厥全埰魏諺 緝?壯妬乙 特隅??禎懿妬諺 增尊有 竣迹增?緝侁
	int nIndex = 0;
	CPPString sTag;
	...
	if (szTable.cx && szTable.cy)
	{
		...
		for (i = 0; i < szTable.cy; ++i)
		{
			...
			if (SearchTag(sTable, nNewRow, _T("tr")))
			{
				...
				//RUS: 躬釣靭咸諺? 壯 茁蚓鏑, 衆抑策陷?裔 適佺妬杖諺 增尊孺
				nNewRow = nEndRow + 5;
			} //if
		} //for
	} //if
	...
	while (nIndex < sRow.GetLength())
	{
		...
		if (nNewCell < nEndRow)
		{
			...
			if (MODE_FIRSTPASS != m_nNumPass)
			{
				...
				//RUS: 淃懿妬諺 釣贍茴渾 穽溢昌鏑茴凹 秧 述脣菴 特隅?
				rcCell = *lpRect;
				...
				//RUS: cellspacing - 剪增蒼 剪 惟粧債?診頌擾?哀 特隅?
//				rcCell.left += m_defStyle.nPadding;
//				rcCell.top += m_defStyle.nPadding;
...
//		sc.bHeightPercent = m_defStyle.bCellHeightPercent;
		//ENG: 
		//RUS: 퇸泣 瘡暹陝?麟杖茵蟻裝??傲儼?特隅? 桎 埰診狀淳潗 撓 魏?壯妬蟻裝?
...
//		sc.szText.cy = m_defStyle.nCellHeight;
	...
	else if (MODE_DRAW == m_nNumPass)
	{
		...
		//RUS: cellpadding - 剪增蒼 剪 惟粧債?特隅? 哀 鎭揄診 純寀鳥 檍
	...
	while(nIndex < sCell.GetLength())
	{
		...
		if (nNearlyTag > nIndex)
		{
			...
			//RUS: 퇸泣 靭輦?禎衆抑杖?妖鴨揄佃 ?鎭有陷?妖鴨揄佃 憎緘增詢奄 鎭揄?
		...
		if (MODE_DRAW != m_nNumPass)
		{
			...
			//RUS: 昆 渟躁佃 ?蕣典佃 穽奠賊性 述婆衆諺 調傲儼?特隅?
...
SIZE CPPHtmlDrawer::DrawHtmlString (CPPString & sHtml, LPCRECT lpRect)
{
	...
	//RUS: 쾰 誼灑?增尊孺 哀蓀瞬諺 緝?<body>
	sHtml = _T("<body>") + sHtml;
	sHtml += _T("</body>");

	//ENG: Bounding rectangle for a full text
	//RUS: 嬌調杖婆循僥 穽溢昌鏑茴凹 秧 述脣菴 蓴憶?鎭揄診
	...
	while (i < sHtml.GetLength())
	{
		...
		if (!sText.IsEmpty())
		{
			...
			case TEXT_TRANSFORM_UPPERCASE:
				...
				//RUS: 찔?茁蚓鏑?渟釣脣怏??橓悰杖?釣燼增?
				sText.MakeUpper();
				break;
			case TEXT_TRANSFORM_LOWERCASE:
				//ENG: All chars make lower
				//RUS: 찔?茁蚓鏑?渟釣脣怏??杖悅僥 釣燼增?
				sText.MakeLower();
				break;
			case TEXT_TRANSFORM_CAPITALIZE:
				//ENG: Each first char of a word to upper
				//RUS: 間液?渟躁渾 茁蚓鏑 衆荻??橓悰杖?釣燼增? 前診蟻裝??杖悅僥
				sText.MakeLower();
			...
			while (!sText.IsEmpty())
			{
				...
				if (MODE_DRAW == m_nNumPass)
				{
					if (sz.cx)
					{
						if ((0 == (nRealWidth - sz.cx)) && (_T(' ') == sTemp.GetAt(sTemp.GetLength() - 1)))
						{
							...
							//RUS: 퇸泣 憙?禎衆抑杖?述脣??增尊劉, 桎 愴寮齧?穽翟言?楫調循
							sTemp.TrimRight();
						...
						//RUS: 煖春陝庾 鎭有姮?翟乙增?魏?翟乙增?燼渟足妖魏 嚴泣 佺 埰診狀瞬孼
						...
						//RUS: 퇸泣 埰診狀瞬孼 增堯?overline, 桎 鳥憎諺 泣杖?壯?鎭揄桎?
				...
				//RUS: 躬釣靭咸諺? 楫調循 剪 述橓鴨牆謫?鎭揄診
				ptOutput.x += sz.cx;
				if (!sText.IsEmpty())
				{
					//ENG: Not all text was printed (cause text wrap) 
					//RUS: 袞 蓴 增尊魏 余?述橓乙贈 (?衆綵齧 渟釣狀焌 鎭揄診)
						...
						case TAG_BITMAP:
							...
							if (si.nIdRes || si.nIdDll || si.nHandle || !si.strSrcFile.IsEmpty())
							{
								...
								else if (si.nHandle)
								{
									...
									//RUS: 퇸泣 瘡暹陝 鴨仲鳥綎典 了翟調蓮杖, 桎 裔穽余齧?昶贍孼猥
									bAutoDelete = FALSE;
								} //if
								
								//ENG: If a handle of an image was retrieved
								//RUS: 퇸泣 鴨仲鳥綎典 了翟調蓮杖 禎懿特?
								if (NULL != hBitmap)
								{
									...
									//RUS: 퇸泣 鎭贓 哀增蒼壯, 桎 埰診壯瞬外齧?釣贍茴渾 調傲儼
						...
						case TAG_IMAGELIST:
							...
							if (si.nIdRes || si.nIdDll || si.nHandle || !si.strSrcFile.IsEmpty())
							{
								...
								if (NULL != hBitmap)
								{
									...
									//RUS: 淃懿妬諺 茵揄夭贍茴赤 婆衆?惟依狀??增尊?了翟調蓮杖?壯 訟嫉涉劉
									nMaxCol = sz.cx / si.cx;
									nMaxRow = sz.cy / si.cy;
...
int CPPHtmlDrawer::InitNewLine(int x)
{
	...
	else if (MODE_DRAW == m_nNumPass)
	{
		...
		//RUS: 淃懿妬諺 菴牆酷 渟躁迹 增尊孺 ?穽蘖鎖暹暢?穽專孼帙策 鋪鳥腸 ?釣贍茴策
...
void CPPHtmlDrawer::SetImageList(HBITMAP hBitmap, int cx, int cy, int nCount, COLORREF crMask /* = RGB(255, 0, 255) */)
{
	...
	//RUS: 擡贍諺 穽抑或蔡僥 楫尿適 了翟調蓮杖?
	if (NULL != m_hImageList)
		::DeleteObject(m_hImageList);

	//ENG: If don't need to create a new image list
	//RUS: 퇸泣 張 腸悅?櫛醴設城?狀述?楫尿適 了翟調蓮杖?
	if (NULL == hBitmap)
		return;
...
CPPString CPPHtmlDrawer::GetTagBody(CPPString & str, int & nIndex)
{
	...
	//RUS: 嵌張?緝侁 壯藕孼?厥全埰魏諺 茁蚓鏑 壯妬乙 緝侁
	nIndex++;
	if (nEndOfTag > nIndex)
	{
...
CPPString CPPHtmlDrawer::SplitTag(CPPString & sTag)
{
	...
	if (tch != _T('\0'))
	{
		...
		//RUS: 及醴言嶢言?壯藕孼. 及醴言諺 鎭依 緝侁 壯 夭 ?檉調靭疾?
...
SIZE CPPHtmlDrawer::AnalyseCellParam(CPPString & sProperties, _STRUCT_CHANGESTYLE & cs, BOOL bTable)
{
	...
	while (i < sProperties.GetLength())
	{
		...
		//RUS: 咬調蓀執循諺 楫業搖料嚴孺?秧 緝侁 <table> 檉調靭疾?
		if(bTable)
		{
...
CPPString CPPHtmlDrawer::GetWordWrap(CPPString & str, int nMaxSize, int & nRealSize)
{
	...
	if (0 == nLastIndex)
	{
		...
		{
			//RUS: 及嗚顥荻 ?增尊劉 張 翟壯宗蓮狀, 禎憙佃?修鴨?調詣外城?增尊有 
			//     禎 茁蚓鏑閃, ?張 禎 衆荻閃
			sz.cx = 0;
			int i = 1;
			...
			//RUS: 袞脣傲跡狀 調詣嶢?增尊有, 述脣怏?墮泣惟?
			sResult = str;
			str.Empty();
		}
		else
		{
			//RUS: ?剪增設袍嚴 靭增?鎭有緘?增尊孺 張 瞬暹嶢 杖 賊狀莘 衆荻?
			sz.cx = 0;
		} //if
	}
	else 
